import type { InferredArray, InferredObject, InferredValue } from '../contracts'
import type { LaravelGeneratorSettings, SettingDefinition } from './types'
...
export interface PhpClassModel {
  rootClassName: string
  records: ClassRecord[]
  classBySignature: Map<string, ClassRecord>
}

export const PHP_SETTING_DEFINITIONS: SettingDefinition[] = [
...
export function getDefaultPhpSettings(): LaravelGeneratorSettings {
  return {
    rootName: 'Root',
    namespace: 'App\\Data',
    addDataSuffix: true,
...
export function normalizeSettings(input: LaravelGeneratorSettings): LaravelGeneratorSettings {
  return {
    ...input,
...
export function buildPhpClassModel(root: InferredObject, settings: LaravelGeneratorSettings): PhpClassModel {
  ...
  ensureRecord(root, rootClassName, normalizedSettings.namespace, classBySignature, records)
  walkObjects(root, nestedNamespaceRoot, normalizedSettings, classBySignature, records)
...
export function resolvePhpType(
  ...
  return {
    typeHint: 'array',
    docblock: `@var list<${record.className}>`
  }
}
...
export function applyCasing(value: string, casing: LaravelGeneratorSettings['propertyCasing']): string {

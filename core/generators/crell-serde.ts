...
export const crellSerdeGenerator: Generator<LaravelGeneratorSettings> = {
  id: {
    language: 'php',
    package: 'crell/serde'
  },
...
function renderRecord(
  ...
  const properties = record.object.properties.map((property) => {
    ...
    const needsFieldAttribute = propertyName !== property.key

    if (needsFieldAttribute) {
      imports.add('Crell\\Serde\\Attributes\\Field')
    }
  ...
  const constructorParams = properties
    .map((property) => {
      ...
      const field = property.needsFieldAttribute
        ? `        #[Field(name: '${property.originalName.replace(/'/g, "\\'")}')]\n`
        : ''
      return `${doc}${field}        public ${property.typeHint} $${property.propertyName}`
    })
    .join(',\n')
